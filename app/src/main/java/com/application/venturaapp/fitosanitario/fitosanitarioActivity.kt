...
class fitosanitarioActivity   : AppCompatActivity(), VSAGRRFITItemListener {
    ...
    var laborList  = arrayListOf<VSAGRRFIT>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tvTituloPersonal.text = "Consumo Fitosanitario"
    ...
    {
        val filter: ArrayList<VSAGRRFIT> = ArrayList()
        ...
        rvLaborCultural.adapter = (FitosanitarioPEPAdapter(this,filter))
    ...
    fun setUpObservers(){
        ...
        fitosanitarioViewModels.VSAGRFIT.observe(this, Observer {
            it?.let { it1 ->
                ...
                */
                laborList = it1 as ArrayList<VSAGRRFIT>
                ...
                rvLaborCultural.adapter = (FitosanitarioPEPAdapter(this, laborList))
                rvLaborCultural.layoutManager = LinearLayoutManager(this)
    ...
    private fun LaborListar() {
        pref.getString(Constants.B1SESSIONID)?.let { fitosanitarioViewModels.listaFitosanitario(it, CodigoPEP,httpCacheDirectory, this) }
    }
    ...
    override fun laborItemClickListener(position: VSAGRRFIT?) {

        var gson =  Gson()
        var jsonClass = gson.toJson(position)

        val intent = Intent(this, FitosanitarioDetalleActivity::class.java)
        intent.putExtra("CODIGOPEP", CodigoPEP)
        intent.putExtra("CAMPANIA", Campania)
        intent.putExtra("CULTIVO", Cultivo)
        intent.putExtra("VARIEDAD", Variedad)
